...
import com.app.touch_of_jasmine.databinding.LayoutEmptyBinding
import com.app.touch_of_jasmine.databinding.LvItemFooterLoadingBinding
import com.app.touch_of_jasmine.helper.Global.showSnackBar
import com.app.touch_of_jasmine.helper.interfaces.CommonInterfaceClickEvent
import com.app.touch_of_jasmine.helper.helper_model.AddImageModel
import com.app.touch_of_jasmine.helper.helper_model.CalenderDatesModel
...
object Extensions {
   ...
   /* fun Context.showCustomSnackBar(
    ...
    fun NestedScrollView.setPaginationListener(
        pageSize: Int,
        layoutFooter: LvItemFooterLoadingBinding,
        currentPg:Int = 1,
        callback: (page: Int) -> Unit,
    ) {
        try {
            var currentPage = currentPg
            if (currentPage < pageSize) {
                currentPage += 1
                callback(currentPage)
                layoutFooter.root.isVisible = currentPage < pageSize
                handler(1000){
                    layoutFooter.root.isVisible = currentPage < pageSize
                }
            }
            this.setOnScrollChangeListener(NestedScrollView.OnScrollChangeListener { _, _, scrollY, _, _ ->
                if (pageSize > 0) {
                    val totalHeight = this.getChildAt(0)?.height ?: 0
                    if (scrollY >= totalHeight - this.height && currentPage < totalHeight / pageSize) {
                        if (currentPage < pageSize) {
                            currentPage += 1
                            callback(currentPage)
                            layoutFooter.root.isVisible = currentPage < pageSize
